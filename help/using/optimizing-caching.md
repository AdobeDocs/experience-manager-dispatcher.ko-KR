...
Dispatcher는 성능을 최적화하는 데 사용할 수 있는 몇 가지 내장 메커니즘을 제공합니다. 이 섹션에서는 캐싱의 이점을 극대화하기 위해 웹 사이트를 디자인하는 방법을 설명합니다.
...
>
>일반적으로 많은 캐싱 전략에는 좋은 URL을 선택하고 이 추가 데이터에 의존하지 않는 것이 포함됩니다.
...
>[!NOTE]
>
>대부분의 레이아웃 측면에서 스타일 시트 및/또는 클라이언트측 스크립트를 사용할 수도 있습니다. 일반적으로 캐싱과 잘 작동합니다.
>
>이 기능은 다음과 같은 URL을 사용할 수 있는 인쇄 버전에도 유용합니다.
>
...
페이지 제목이나 기타 텍스트를 사진으로 렌더링한 경우 페이지의 콘텐츠 업데이트 시 삭제되도록 파일을 저장합니다.
...
[고정 연결](dispatcher.md#TheBenefitsofLoadBalancing) 은 한 명의 사용자에 대한 문서가 모두 동일한 서버에서 구성되도록 합니다. 사용자가 이 폴더를 떠났다가 나중에 다시 돌아와도 연결은 계속 유지됩니다. 웹 사이트에 고정 연결이 필요한 모든 문서를 보관할 수 있도록 폴더 하나를 정의합니다. 다른 문서는 여기에 넣지 마십시오. 이는 개인화된 페이지 및 세션 데이터를 사용하는 경우 로드 밸런싱에 영향을 미칩니다.

## MIME 유형 {#mime-types}

브라우저가 파일 유형을 결정할 수 있는 두 가지 방법이 있습니다.

1. 확장명(예: .html, .gif 및 .jpg)별
...
* download.jsp?file=2214와 같은 URL이 있는 일반 파일 제공 스크립트를 사용하지 마십시오. 파일 사양이 포함된 URL을 사용하도록 스크립트를 재작성합니다. 이전 예제의 경우 다음과 같습니다. `download.2214.pdf`.
