---
title: Dispatcher 설치
description: Microsoft&reg; Internet Information Server, Apache 웹 서버 및 Sun Java&trade; 웹 서버-iPlanet에 Dispatcher 모듈을 설치하는 방법에 대해 알아봅니다.
contentOwner: User
converted: true
...
예를 들어 `dispatcher-apache2.4-linux-x86_64-ssl-4.3.1.tar.gz` 파일에는 Linux® i686에서 실행되는 Apache 2.4 웹 서버용 Dispatcher 버전 4.3.1이 포함되어 있으며 파일은 다음을 사용하여 패키지됩니다. **tar** 포맷.
...
| Microsoft® 인터넷 정보 서버 7.5, 8, 8.5, 10 | dispatcher-**iis**-&lt;기타 매개변수> |
| Sun Java™ 웹 서버 iPlanet | dispatcher-**ns**-&lt;기타 매개변수> |

>[!CAUTION]
>
>플랫폼에 사용할 수 있는 최신 버전의 Dispatcher를 설치해야 합니다. 제품 개선 사항을 활용하려면 매년 최신 버전을 사용하도록 Dispatcher 인스턴스를 업그레이드합니다.

>[!NOTE]
>
>버전 4.3.3에서 버전 4.3.4로 특별히 업그레이드하는 고객은 캐시할 수 없는 콘텐츠에 대해 캐싱 헤더를 설정하는 방법이 다르다는 것을 인지해야 합니다. 이 변경 사항에 대해 자세히 알아보려면 [릴리스 정보](/help/using/release-notes.md#nov) 페이지를 가리키도록 업데이트하는 중입니다.
...
Dispatcher 설치를 구성하려면 다음을 편집합니다 `disp_iis.ini` 파일. `.ini` 파일의 기본 형식은 다음과 같습니다.
...
| loglevel | 이벤트 로그에 메시지를 출력하는 데 사용되는 로그 수준을 정의합니다. 로그 파일의 로그 수준에서 다음 값을 지정할 수 있습니다. <br/>0 - 오류 메시지만 표시합니다. <br/>1 - 오류 및 경고. <br/>2 - 오류, 경고 및 정보 메시지 <br/>3 - 오류, 경고, 정보 메시지 및 디버그 메시지. <br/>**참고**: 설치 및 테스트 중에는 로그 수준을 3으로 설정하고 프로덕션 환경에서 실행할 때는 0으로 설정합니다. |
...
1. Dispatcher 폴더의 권한 대화 상자에서 방금 추가한 계정을 선택하고 계정에 대한 모든 권한을 활성화합니다  **전체 제어 제외** 확인을 클릭합니다. 확인을 클릭하여 폴더 속성 대화 상자를 닫습니다.
...
1. 로그 파일의 위치를 구성하려면 다음 예제와 유사한 텍스트 행을 추가한 다음 파일을 저장합니다.
...
Dispatcher 사용을 시작하려면 먼저 다음을 알아야 합니다.

* Dispatcher [구성](dispatcher-configuration.md)
* [AEM 구성](page-invalidate.md) 를 사용하여 Dispatcher에서 작업할 수 있습니다.

## Apache 웹 서버 {#apache-web-server}

>[!CAUTION]
>
>다음 두 항목 아래에 설치 지침 **Windows** 및 **UNIX®** 여기 있습니다. 단계를 수행할 때 주의하십시오.
...
>[!CAUTION]
>
>소스 파일을 컴파일하여 Apache 바이너리를 생성하는 경우 를 설정해야 합니다 **`dynamic modules support`**. 이 작업을 **--enable-shared** 옵션을 사용하여 수행할 수 있습니다. 최소한 다음을 포함합니다. `mod_so` 모듈.
>
...
설치 아카이브 파일에는 Windows 또는 UNIX 선택 여부에 따라 다음 파일이 포함되어 있습니다®
...
1. Dispatcher 파일을 적절한 Apache 모듈 디렉터리에 배치합니다.
   ...
   * **UNIX®**: 다음 중 하나를 찾습니다 `<APACHE_ROOT>/libexec` 또는 `<APACHE_ROOT>/modules`설치에 따라 디렉토리를 지정합니다.\
     `dispatcher-apache<options>.so`를 이 디렉터리에 복사합니다.\
     장기 유지 관리를 단순화하기 위해 라는 심볼 링크를 만들 수도 있습니다 `mod_dispatcher.so` Dispatcher로:\
...
터미널 창에 다음 명령을 입력하고 `[path to the dispatcher.so file]` Apache 웹 서버에 설치한 Dispatcher 모듈의 경로 및 *`path to the docroot`* docroot가 있는 경로(예: `/opt/cq/cache`):
...
다음을 사용하여 Apache 웹 서버를 구성해야 합니다. `httpd.conf`. Dispatcher 설치 키트에서 라는 예제 구성 파일을 찾습니다 `httpd.conf.disp<x>`.
...
1. 나열된 순서대로 다음 구성 항목을 추가해야 합니다.
   ...
   * **모듈 로드** 시작할 때 모듈을 로드합니다.
   * 다음을 포함한 Dispatcher 특정 구성 항목 **DispatcherConfig, DispatcherLog**, 및 **DispatcherLoglevel**.
...
Dispatcher별 구성 항목은 LoadModule 항목 뒤에 배치됩니다. 다음 표에는 UNIX® 및 Windows 모두에 적용할 수 있는 예제 구성이 나와 있습니다.
...
| DispatcherLogLevel | 로그 파일의 로그 수준: <br/>0 - 오류 <br/>1 - 경고 <br/>2 - 정보 <br/>3 - 디버그 <br/>**참고**: 설치 및 테스트 중에는 로그 수준을 3으로 설정하고 프로덕션 환경에서 실행할 때는 0으로 설정합니다. |
| DispatcherNoServerHeader | *이 매개 변수는 더 이상 사용되지 않으며 효과도 없습니다.*<br/><br/> 사용할 서버 헤더를 정의합니다. <br/><ul><li>정의되지 않음 또는 0 - HTTP 서버 헤더에 AEM 버전이 포함됩니다. </li><li>1 - Apache 서버 헤더가 사용됩니다.</li></ul> |
| DispatcherDeclineRoot | 루트 &quot;/&quot;에 대한 요청을 거절할지 여부를 정의합니다. <br/>**0** - /(으)로의 요청 수락 <br/>**1** - /에 대한 요청은 Dispatcher에 의해 처리되지 않습니다. 올바른 매핑을 위해 mod_alias를 사용하십시오. |
| DispatcherUseProcessedURL | Dispatcher의 모든 추가 처리에 사전 처리된 URL을 사용할지 여부를 정의합니다. <br/>**0** - 웹 서버에 전달된 원래 URL을 사용합니다. <br/>**1** - Dispatcher는 Dispatcher 앞에 오는 핸들러에 의해 이미 처리된 URL(즉, `mod_rewrite`)을 클릭하여 웹 서버에 전달할 수 있습니다. 예를 들어 원본 또는 처리된 URL이 Dispatcher 필터와 일치합니다. URL은 캐시 파일 구조의 기초로도 사용됩니다. mod_rewrite에 대한 정보는 Apache 웹 사이트 설명서를 참조하십시오(예: Apache 2.4). mod_rewrite를 사용할 때 &#39;passthrough&#39;(다음 핸들러로 전달) 플래그를 사용하여 재작성 엔진이 내부 request_rec 구조의 URI 필드를 파일 이름 필드의 값으로 설정하도록 합니다. |
| DispatcherPassError | ErrorDocument 처리를 위해 오류 코드를 지원하는 방법을 정의합니다. <br/>**0** - Dispatcher가 클라이언트에 대한 모든 오류 응답을 스풀 처리합니다. <br/>**1** - Dispatcher가 클라이언트에 오류 응답을 스풀 처리하지 않습니다(상태 코드가 400보다 크거나 같은 경우). 대신 상태 코드를 Apache에 전달하여 ErrorDocument 지시문이 이러한 상태 코드를 처리할 수 있도록 합니다. <br/>**코드 범위** -응답이 Apache에 전달되는 오류 코드의 범위를 지정합니다. 다른 오류 코드는 클라이언트에 전달됩니다. 예를 들어 다음 구성은 오류 412에 대한 응답을 클라이언트에 전달하고 다른 모든 오류는 Apache에 전달합니다. DispatcherPassError 400-411,413-417 |
...
>
>통계적 목적으로 AEM 버전을 표시합니다. 헤더에서 이러한 정보를 사용할 수 없도록 비활성화하려면 다음을 설정할 수 있습니다.
>
>`ServerTokens Prod`
>
...
다음 이후 **SetHandler** 명령문입니다. **ModMimeUsePathInfo** 정의.

>[!NOTE]
>
>를 사용하고 구성합니다. `ModMimeUsePathInfo` 매개 변수 : Dispatcher 버전 4.0.9 이상을 사용하는 경우 입니다.
>
...
mod_mime 모듈(예: [Apache 모듈 mod_mime](https://httpd.apache.org/docs/2.4/mod/mod_mime.html))는 HTTP 응답을 위해 선택한 콘텐츠에 콘텐츠 메타데이터를 할당하는 데 사용됩니다. 기본 설정은 mod_mime이 콘텐츠 유형을 결정할 때 파일이나 디렉터리에 매핑되는 URL 부분만 고려됨을 의미합니다.

날짜 `On`, `ModMimeUsePathInfo` 매개 변수는 `mod_mime` 을(를) 통해 콘텐츠 유형을 *완료* URL; 즉, 가상 리소스에는 해당 확장을 기반으로 메타 정보가 적용됩니다.
...
이러한 웹 서버를 설치하는 방법에 대한 전체 정보는 해당 설명서를 참조하십시오.
...
웹 서버는 다음을 사용하여 구성해야 합니다. `obj.conf`. Dispatcher 설치 키트에서 라는 예제 구성 파일을 찾습니다 `obj.conf.disp`.
...
| loglevel | 로그 파일에 메시지를 작성할 때의 로그 수준: <br/>**0** 오류 <br/>**1** 경고 <br/>**2** 정보 <br/>**3** 디버그 <br/>**참고:** 설치 및 테스트 중에는 로그 수준을 3으로 설정하고 프로덕션 환경에서 실행할 때는 0으로 설정합니다. |
| keepalivetimeout | keep-alive 시간 제한을 초 단위로 지정합니다. Dispatcher 버전 4.2.0부터 기본 keep-alive 값은 60입니다. 값이 0이면 keep-alive가 비활성화됩니다. |

요구 사항에 따라 Dispatcher를 개체에 대한 서비스로 정의할 수 있습니다. 전체 웹 사이트에 대해 Dispatcher를 구성하려면 기본 개체를 편집합니다.
