---
title: Dispatcher 개요
description: AEM Cloud Services에서의 개선된 보안, 캐싱 등을 위해 Adobe Experience Manager Dispatcher를 사용하는 방법을 알아봅니다.
...
이 프로세스는 간단하고 효율적입니다. 방문자가 HTML 페이지와 같은 파일을 요청하는 경우 파일은 대개 메모리에서 직접 가져오며, 최악의 경우 로컬 드라이브에서 읽습니다. 정적인 웹 서버는 꽤 오랫동안 제공되어 왔습니다. 따라서 관리와 보안 관리를 위한 다양한 도구가 있습니다. 이러한 도구는 네트워크 인프라와 잘 통합되어 있습니다.
...
>
>파일 이름 길이에 대한 OS 수준의 제한이 있을 수 있습니다. 선택기가 많은 URL이 있는 경우입니다.
...
1. 캐시에서 동일한 핸들로 시작하는 모든 파일을 삭제합니다. 예를 들어 파일 `/en/index.html`이 업데이트되면 `/en/index.`로 시작하는 모든 파일이 삭제됩니다. 이 메커니즘을 통해 특히 사진 탐색을 위한 캐시 효율적인 사이트를 설계할 수 있습니다.
1. 소위 *statfile*&#x200B;을 **터치**&#x200B;합니다. 그러면 상태 파일의 타임스탬프가 업데이트되어 마지막 변경 날짜의 날짜가 표시됩니다.

다음 요점을 명시해야 합니다.

* 일반적으로 콘텐츠 업데이트는 대체해야 하는 항목을 “아는” 작성 시스템과 함께 사용됩니다.
* 콘텐츠 업데이트의 영향을 받는 파일은 제거되지만 즉시 대체되지 않습니다. 다음에 해당 파일이 요청되면 Dispatcher은 AEM 인스턴스에서 업데이트된 버전을 검색하고 캐시된 사본을 대체합니다.
...
Dispatcher는 캐시된 파일을 정적 웹 사이트의 일부인 것처럼 웹 서버에 저장합니다. 사용자가 캐싱 가능한 문서를 요청하는 경우 Dispatcher는 해당 문서가 웹 서버의 파일 시스템에 있는지 여부를 확인합니다.
...
* **향상된 처리 능력**
실제로 처리 능력이 향상되었다는 것은 Dispatcher이 AEM의 여러 인스턴스 간에 문서 요청을 공유한다는 것을 의미합니다. 이제 각 인스턴스에 처리할 문서가 적으므로 응답 시간이 빨라집니다. Dispatcher는 각 문서 범주에 대한 내부 통계를 유지하므로 로드를 예측하고 쿼리를 효율적으로 배포할 수 있습니다.

* **실패 시 보장 범위 증가**
Dispatcher이 인스턴스로부터 응답을 받지 못하면 자동으로 요청을 다른 인스턴스 중 하나로 전달합니다. 인스턴스를 사용할 수 없게 되면 그 유일한 효과는 계산 능력이 손실되는 정도에 비례한 사이트의 둔화입니다. 단, 모든 서비스는 계속 제공됩니다.
...
HTTP 인프라 구성 요소로서 콘텐츠 전송 네트워크는 Dispatcher와 매우 유사하게 작동합니다. 콘텐츠 전송 네트워크 노드가 요청을 받으면 가능한 경우 캐시에서 요청을 처리합니다(리소스는 캐시에서 사용할 수 있으며 유효함). 그렇지 않으면 리소스를 검색하고 적절한 경우 추가 요청을 위해 캐시하기 위해 다음으로 가장 가까운 서버에 연결됩니다.
...
1. 만료 및 캐시 제어 헤더입니다.
대부분의 CDN은 업스트림 서버에서 보내는 경우 `Expires:` 및 `Cache-Control:` HTTP 헤더를 처리합니다. 이 메서드는 예를 들어 [mod_expires](https://httpd.apache.org/docs/2.4/mod/mod_expires.html) Apache 모듈을 사용하여 수행할 수 있습니다.
...
이 방법을 사용하여 관리 콘텐츠를 캐싱하는 경우 구성된 캐싱 기간이 만료되고 콘텐츠 변경 사항이 최종 사용자에게 표시된다는 의미입니다. 문서를 다시 Dispatcher에서 가져오는 경우에 해당됩니다.
...
1. 웹 서버를 다시 시작합니다.
   ...
   >[!NOTE]
   >
   >제공된 `author_dispatcher.any` 구성을 통해 `/libs` 또는 `/apps`에서 콘텐츠에 영향을 주는 CQ5 기능 팩, 핫픽스 또는 애플리케이션 코드 패키지를 설치하는 경우 캐시된 파일을 삭제해야 합니다. 파일은 Dispatcher 캐시의 해당 디렉터리에 있습니다. 이렇게 하면 다음 번에 요청될 때 이전에 캐시된 항목이 아닌 새로 업그레이드된 파일을 가져오게 됩니다.
