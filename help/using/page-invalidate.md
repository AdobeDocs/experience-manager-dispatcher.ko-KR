...
AEM 작성자 인스턴스의 복제 에이전트는 페이지가 게시될 때 캐시 무효화 요청을 Dispatcher에 보냅니다. Dispatcher는 새 콘텐츠가 게시되면 결국 캐시에서 파일을 새로 고칩니다.
...
다음 절차를 사용하여 AEM 작성자 인스턴스에서 복제 에이전트를 구성합니다. 구성은 페이지 활성화 시 Dispatcher 캐시를 무효화합니다.
...
1. 전송 탭에서 URI를 입력하여 Dispatcher에 액세스합니다.
...
>[!NOTE]
>
>Dispatcher 캐시를 플러시하기 위한 에이전트에는 사용자 이름과 암호가 필요하지 않지만 구성된 경우 기본 인증을 사용하여 전송됩니다.

이 접근 방식에는 두 가지 잠재적인 문제가 있습니다.

* 작성 인스턴스에서 Dispatcher에 연결할 수 있어야 합니다. 네트워크(예: 방화벽)가 둘 사이의 액세스가 제한되도록 구성된 경우에는 그렇지 않을 수 있습니다.

* 게시 및 캐시 무효화는 동시에 발생합니다. 시기에 따라 사용자는 캐시에서 페이지가 제거된 직후와 새 페이지가 게시되기 직전에 페이지를 요청할 수 있습니다. AEM은 바로 이때 이전 페이지를 반환하고 Dispatcher는 이를 다시 캐시합니다. 이러한 상황은 대규모 사이트의 경우 더 큰 문제가 됩니다.
...
* AEM Dispatcher와 게시 인스턴스 간에 가능한 시간 충돌이 발생하지 않도록 합니다( 참조) [작성 환경에서 Dispatcher 캐시 무효화](#invalidating-dispatcher-cache-from-the-authoring-environment)).
* 시스템에는 고성능 서버에 상주하는 여러 개의 게시 인스턴스와 한 개의 작성 인스턴스만 포함되어 있습니다.

>[!NOTE]
>
>숙련된 AEM 관리자가 이 메서드를 사용할 것인지 결정해야 합니다.

게시 인스턴스에서 작동하는 복제 에이전트는 Dispatcher 플러시를 제어합니다. 단, 구성은 작성 환경에서 수행된 다음 에이전트를 활성화하여 전송됩니다.

1. AEM 도구 콘솔을 엽니다.
1. 게시의 도구/복제/에이전트 아래에서 필요한 복제 에이전트를 엽니다. 기본적으로 설치되어 있는 Dispatcher 플러시 에이전트를 사용할 수 있습니다.
1. 편집을 클릭하고 설정 탭에서 **사용**&#x200B;이 선택되었는지 확인합니다.
1. (선택 사항) 별칭 또는 vanity 경로 무효화 요청을 활성화하려면 **별칭 업데이트** 옵션을 선택합니다.
1. 전송 탭에서 필요한 URI를 입력하여 Dispatcher에 액세스합니다.\
   표준 Dispatcher 플러시 에이전트를 사용하는 경우 호스트 이름과 포트를 업데이트합니다. 예: `http://<dispatcherHost>:<portApache>/dispatcher/invalidate.cache`
...
* 이름이 인 모든 디렉토리 `_jcr_content` 아래 `en` 디렉터리(존재하는 경우, 페이지의 하위 노드에 대한 캐시된 렌더링 포함)
...
추가 헤더 `CQ-Action-Scope: ResourceOnly`를 전송하여 무효화(즉, .stat 파일 터치)를 방지할 수 있습니다. 이 기능은 특정 리소스를 플러시하는 데 사용할 수 있습니다. JSON 데이터와 같은 캐시의 다른 부분을 무효화하지 않고 모든 작업을 수행할 수 있습니다. 이 데이터는 동적으로 생성되며, 캐시와 독립적으로 정기적인 플러시가 필요합니다. 예를 들어 뉴스, 주식 시세 표시기 등을 표시하기 위해 서드파티 시스템에서 얻은 데이터를 나타냅니다.
